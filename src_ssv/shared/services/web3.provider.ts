...
export class Web3Provider {
  ...
  constructor(private _config: ConfService) {
    ...
    try {
      ...
      jsonCoreData = require(`@cli-ssv/shared/abi/${contractEnv}.${contractGroup}.abi.json`);
    } catch (err) {
      console.error(
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      jsonViewsData = require(`@cli-ssv/shared/abi/${contractEnv}.${contractGroup}.views.abi.json`);
    } catch (err) {
      console.error(
